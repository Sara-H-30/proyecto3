...
async def consultar_resenas_hotel(hotel_id: str, sort_by: str = "fecha", skip: int = 0, limit: int = 10):
    ...
    # 1. QUITAMOS el {"_id": 0} en ambas búsquedas
    ...
    cursor = reviews_collection.find(query).sort(sort_field, -1).skip(skip).limit(limit)
    
    todas = destacada + list(cursor)

    # 2. TRUCO VITAL: Convertimos el ObjectId a texto normal para que APEX no se confunda
    for r in todas:
        r["_id"] = str(r["_id"])
